...
class ExternalFeatures:
    ...
    def __init__(self, verbose=False):
        ...
        # Period in which to fetch data back in time
        self.start = '1990-01-01'

    def update(self):
        logging.info('Updating external features')
        self.all_converged = list()
        ...
        if not all(self.all_converged):
            logging.warning('External features was not updated due to convergence issue')
            return
        ...
        try:
            ...
            _ext_path = '/home/pi/Documents/STA2-DEPLOY/data/external_features/external_features.csv'
            df.to_csv(_ext_path ,index=True)

        logging.info('External features have been updated')
    ...
    def _gen_yield_curve(self):
        ...
        treas_bill_short = treas_bill_short_ticker.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
        treas_bill_5y = treas_bill_5y_ticker.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
        treas_bill_10y = treas_bill_10y_ticker.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
        treas_bill_30y = treas_bill_30y_ticker.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
        ...
        if max(df_yield.yield_curve.to_list()) > 1 or min(df_yield.yield_curve.to_list()) < -1:
            logging.error(f'ERROR: Yield curve produced large/small values (max:{max(df_yield.yield_curve.to_list())} min:{min(df_yield.yield_curve.to_list())}). You have to scale them.')
    ...
    def _gen_gold(self):
        ...
        gold = gold_tick.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
    ...
    def _gen_oil(self):
        ...
        oil = oil_tick.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
    ...
    def _gen_eurusd(self):
        ...
        eurusd = eurusd_tick.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
    ...
    def _gen_dji(self):
        ...
        dji = dji_tick.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
    ...
    def _gen_snp(self):
        ...
        snp = snp_tick.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
    ...
    def _gen_nasdaq(self):
        ...
        nasdaq = nasdaq_tick.history(start=self.start, end=str(datetime.now().date()), interval='1d').Close
    ...
    def _process(self, data):
        ...
        if not converged:
            logging.error(f'Fractional differencing for external feauture was not able to converge for {feature_name}')
            self.all_converged.append(False)
        else:
            self.all_converged.append(True)
...
if __name__ == '__main__':
    exfeat = ExternalFeatures(verbose=True)
    exfeat.update()